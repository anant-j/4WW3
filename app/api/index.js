...
app.post('/register', async (req, res) => {
    ...
    try {
        const [rows] = await connection.query(
            'INSERT INTO Users (Email, Password, FirstName, LastName, DOB, Latitude, Longitude) VALUES (?,?,?,?,?,?,?)', [email, hashedPassword, firstName, lastName, dob, latitude, longitude]
        )
        const user = new User(email, firstName, lastName, dob, latitude, longitude);
...
app.post('/addRestaurant', async (req, res) => {
    ...
    try {
        const [rows] = await connection.query(
            'INSERT INTO Restaurants (Name, About, Phone, Website, Image, Latitude, Longitude) VALUES (?,?,?,?,?,?,?)', [name, description, phone, website, image, latitude, longitude]
        )
        if (rows.affectedRows === 1) {
            res.send({
                success: true,
